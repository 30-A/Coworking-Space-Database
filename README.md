...
The system is designed to be **ACID-compliant**, ensuring data integrity during reservations (preventing double bookings), and uses **Triggers** for automatic audit logging.
...
| File | Description |
|---|---|
| 01\_create\_tables.sql | Creates database schema with Primary and Foreign Keys |
| 02\_import\_data.sql | Imports bulk data from CSV files |
| 03\_create\_indexes.sql | Creates indexes for performance optimization |
| 04\_crud\_operations.sql | Basic operations (INSERT, SELECT, UPDATE, DELETE) |
| 05\_joins\_and\_aggregations.sql | Analytical queries using JOINs and Grouping |
| 06\_stored\_procedures.sql | Reusable procedures for reports and history |
| 07\_triggers.sql | Automation for logging booking changes |
| 08\_transactions.sql | ACID-compliant transaction to prevent double bookings |
